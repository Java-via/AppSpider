...
def get_html_content(response, charset=None):
    """
    get html content from a response, charset can be "utf-8", "gb2312" and "gbk", etc
    """
...
def get_json_data(string, pattern=None, annotation_pattern=None, begin_pattern=None):
    """
    get json data from string, using pattern to extract
...
def get_string_split(string, split_chars=(" ", "\t", ","), is_remove_empty=False):
    """
    get string list by splitting string from split_chars, len(split_chars) must >= 2
    """
